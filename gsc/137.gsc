...
thundergun_shoot()
{
	view_pos = self getTagOrigin( "tag_weapon_left" );
    axis_array = getaiarray( "axis" );

    if( ! isdefined( axis_array ) )
    {
        return;
    }

    foreach( target in axis_array )
    {
        ...
        if( distance( target.origin, view_pos ) > 1200 )
        {
            continue;
        }

        thundergun_shoot_target( target, view_pos, self );
    }
}
...
thundergun_shoot_target( target, view_pos, player )
{
    ...
    if ( DistanceSquared( test_origin, radial_origin ) > 32400 )
    {
        return;
    }

    if ( 0 == target DamageConeTrace( view_pos, target ) )
    {
        return;
    }
    ...
    while( 1 )
    {
        ...
        switch( weapon )
        {
            case "t5_thundergun_mp":
                playFX( level.fx_thundergun, self getTagOrigin( "tag_weapon_left" ) );

                PhysicsExplosionCylinder( self.origin, 600, 240, 1 );

                self thread thundergun_shoot();
            break;
        }
    }
}
...
_id_17D0()
{
    ...
    foreach ( var_1 in level.players )
    {
        ...
        var_1 thread watch_weapon_fire();
    }
